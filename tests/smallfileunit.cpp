...
WriterUnit::setUp() {
    ...
    writer = new SMF_Writer(namefile, 0);
    return;
}
...
verifyIndexOperation(const IndexOperation &op) {
    ...
    len = read(op.fd, &index, sizeof index);
    int leng = len;
...
WriterUnit::writefileTest() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (nop.fd == -1) {
            ...
            fd = open(indexfile.c_str(), O_RDONLY);
            iop.fd = fd;
        }
...
NamesMappingUnit::loadfileTest() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        writers.push_back(SMF_Writer(namefile, i));
