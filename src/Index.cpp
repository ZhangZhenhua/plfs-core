...
IndexFileInfo::listToStream(vector<IndexFileInfo> &list,int *bytes)
{
    ...
    for(itr=list.begin(); itr!=list.end(); itr++) {
        ...
        char *path = strdup((*itr).path.c_str());
        ...
        buf_pos=memcpy_helper(buf_pos,(void *)path,path_len);
        free(hostname);
        free(path);
    }
    return (void *)buffer;
...
IndexFileInfo::streamToList(void *addr)
{
    ...
    for(count=0; count<size; count++) {
        ...
        path_ptr=(char *)addr;
        path.append(path_ptr);
        index_dropping.path=path;
...
ostream& operator <<(ostream& os,const Index& ndx )
{
    ...
    for(unsigned i = 0; i < ndx.chunk_map.size(); i++ ) {
        if(ndx.chunk_map[i].path.find(DATAPREFIX) != string::npos){
...
Index::flush()
{
    ...
    if ( len != 0 ) {
        ...
        ret     = Util::Writen( getFd(BYTERANGE), start, len );
        if ( (size_t)ret != (size_t)len ) {
            mlog(IDX_DRARE, "%s failed write to fd %d: %s",
                __FUNCTION__, getFd(BYTERANGE), strerror(errno));
        }
        byteRangeIndex.clear();
    }
    ...
    // so it's ok to fail to get index fd for other ranks
    int index_fd = getFd(SIMPLEFORMULA);
    len = simpleFormulaIndex.size() *sizeof(SimpleFormulaEntry);
    if ( index_fd <= 0 || len == 0 ) {
        return 0;
    }
    start = &(simpleFormulaIndex.front());
    ret = Util::Writen( getFd(SIMPLEFORMULA), start, len);
    if ( (size_t)ret != (size_t)len ) {
        mlog(IDX_DRARE, "%s failed write to fd %d: %s",
             __FUNCTION__, getFd(BYTERANGE), strerror(errno));
    }
...
int Index::readIndex( string hostindex )
{
    ...
    if (hostindex.find(FORMULAINDEXPREFIX) != string::npos){
        ...
        for( size_t i = 0; i < entries; i++ ) {
            ...
            // will be filled in
            cf.path = hostindex.substr(0,hostindex.find(HOSTDIRPREFIX));
            cf.fd = -1;
            m_entry.nw                   = entry.nw;
                         ...
                         off_t logical )
{
    ...
    *chunk_id = (pid_t)-1;
    ...
    if ( global_byteRange_index.size() == 0 ) {
        ...
        *hole = true;
        ...
        if ( entry.contains( logical ) ) {
            ...
            //mlog(IDX_DCOMMON, "%s", oss.str().c_str() );
            chunkFound( fd, chunk_off, chunk_len,
    ...
    // Here we have got <chunk_off, chunk_len, path, chunk_id, ts, path, hole>
    // tuple from ByteRange Index
    // if hole is set, then chunk_off, path, chunk_id is not initilized
    ...
    if (found){
        ...
        ChunkFile *cf_ptr = &(chunk_map[*chunk_id]); // typing shortcut
        *fd          = cf_ptr->fd;
        *chunk_off   = p_off;
        *chunk_len   = len;
        if (cf_ptr->path.find(FORMULADATAPREFIX) == string::npos){
            // The data file locates in shadow container or canonical container
            string mlink = Container::getMetalinkPath(cf_ptr->path,pid,
                                                      metalinkTime);
            string resolved;
            if( Container::resolveMetalink(mlink,resolved) == 0 ){
                ostringstream tmp;
                tmp.setf(ios::fixed,ios::floatfield);
                tmp << resolved << "/" << FORMULADATAPREFIX << ts << "." << pid;
                path = tmp.str();
            }else{
                path = Container::getDataPath(cf_ptr->path,Util::hostname(),
                 ...
                 double begin_timestamp, double end_timestamp )
{
    ...
    if (buffering && !buffer_filled) {
        ...
        if(chunk_map.size()==0) {
            ChunkFile cf;
            cf.fd = -1;
            int fd = current_fd[BYTERANGE];
            string path = index_paths[fd];
            cf.path = Container::chunkPathFromIndexPath(path,pid);
...
{
    string indexfile = index_paths[fd];
    map< double, SimpleFormulaInMemEntry >::iterator sitr;
