...
void
Index::setCurrentFh(IOSHandle *fh , string indexpath )
{
   if (indexpath.find(FORMULAINDEXPREFIX) != string::npos){
      current_fh[SIMPLEFORMULA] = fh;
   } else if (indexpath.find(BYTERANGEINDEXPREFIX) != string::npos){
      current_fh[BYTERANGE] = fh;
   }
   index_paths[fh] = indexpath;
}
    ...
    if (found){
        ...
        *chunk_len   = len;
        if (cf_ptr->bpath.find(FORMULADATAPREFIX) == string::npos){
            ...
            string bpath;
            int rv = plfs_phys_backlookup(path.c_str(), NULL, &backout, &bpath);
            if( rv != 0 ){
                 ...
                 double begin_timestamp, double end_timestamp )
{
    ...
    if (buffering && !buffer_filled) {
        ...
        if(chunk_map.size()==0) {
            ChunkFile cf;
            cf.fh = NULL;
            IOSHandle *fh = current_fh[BYTERANGE];
            string path = index_paths[fh];
