...
int
Small_fd::rename(const char *path) {
    return 0;
}
...
int
Small_fd::getChunkFd(pid_t chunk_id) {
    map<pid_t, int>::iterator itr;
    itr = container->chunk_map.find(chunk_id);
    if (itr == container->chunk_map.end()) return -1;
    return itr->second;
}
int
Small_fd::setChunkFd(pid_t chunk_id, int fd) {
    container->chunk_map[chunk_id] = fd;
    return 0;
}

int
Small_fd::globalLookup(int *fd, off_t *chunk_off, size_t *length,
                       ...
                       off_t logical)
{
    ...
    *hole = (entry.offset == HOLE_PHYSICAL_OFFSET);
    if (!*hole && entry.did != HOLE_DROPPING_ID) {
        container->get_data_file(entry.did, path);
    }
    return 0;
}
