...
int
find_read_tasks(PLFSIndex *index, list<ReadTask> *tasks, size_t size, off_t offset,
                char *buf)
{
    int ret;
...
perform_read_task( ReadTask *task, PLFSIndex *index )
{
    ...
    if ( task->hole ) {
        ...
        if ( task->fh == NULL ) {
            ...
            if ( task->fh == NULL ) {   // not currently stashed, we have to open it
...
ssize_t
plfs_reader(void *unused, char *buf, size_t size, off_t offset,
            PLFSIndex *index)
{
