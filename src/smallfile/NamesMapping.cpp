...
public:
    NameReader(const string &fname, vector<string>::size_type id,
               int buf_size);
...
NameReader::NameReader(const string &fname, vector<string>::size_type id,
...
NamesMapping::init_data_source(void *resource, RecordReader **reader) {
    const vector<string> &files = *(vector<string> *)resource;
    ...
    *reader = min_heap;
    ...
    for (i = 0; i < files.size(); i++) {
        ...
        mlog(SMF_DAPI, "Load names from %s.", files[i].c_str());
        ret = namefile->pop_front();
        if (ret == 1 && namefile->front()) {
            min_heap->push_back(namefile);
        } else if (ret == 0) { // Skip empty files.
            delete namefile;
            mlog(SMF_DAPI, "Skip empty name file:%s.", files[i].c_str());
        } else {
            delete namefile;
            mlog(SMF_WARN, "Error read name file:%s.", files[i].c_str());
        }
    }
    return 0;
}
...
int
NamesMapping::read_names(set<string> *res, vector<string> *namefiles) {
    map<string, FileMetaDataPtr>::iterator names_itr;
    int ret;
