...
SMF_Writer::SMF_Writer(const string &fname, ssize_t did) : filename(fname) {
    dropping_id = did;
...
SMF_Writer::add_resource(int type, void *resource) {
    ...
    case WRITER_OPENDATAFILE:
        if (!data_file.is_opened()) {
            string dataname;
            ret = dropping_name2data(filename, dataname);
            if (!ret) ret = data_file.open_file(dataname.c_str());
            if (ret) break;
        }
        /* If data file has been opened, clear 'filename' to save memory */
        clear_filename = true;
    case WRITER_OPENINDEXFILE:
        if (!index_file.is_opened()) {
            string indexname;
            ret = dropping_name2index(filename, indexname);
    ...
    case WRITER_OPENNAMEFILE:
        if (!name_file.is_opened()) {
            if (!filename.empty()) {
                ret = name_file.open_file(filename.c_str());
            } else {
                ret = -EINVAL;
            }
        }
    ...
    if (!ret && clear_filename) filename.clear();
    return ret;
}
