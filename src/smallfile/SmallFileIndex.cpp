...
public:
    IndexReader(const string &fname, const index_mapping_t &meta, int bufsize);
...
IndexReader::IndexReader(const string &fname, const index_mapping_t &meta,
...
SmallFileIndex::SmallFileIndex(void *init_para) {
    ...
    if (!ret) release(MEMCACHE_FULLYLOADED, init_para);
}
...
{
    int ret = 0;
    vector<string> &droppings = *(((index_init_para_t *)init_para)->namefiles);
    ...
    for (itr = fid->begin(); itr != fid->end(); itr++ ) {
        ...
        ret = dropping_name2index(droppings[itr->second], index_fname);
        if (ret) {
            mlog(SMF_ERR, "Unable to get index file name from name file:%s.",
                 droppings[itr->second].c_str());
            break;
        }
        indexfile = new IndexReader(index_fname, *itr, buf_size);
        /* Only after this pop_front(), we can get the first record. */
        pop_result = indexfile->pop_front();
        if (pop_result == 1 && indexfile->front()) {
            mlog(SMF_DAPI, "Load index entries from %s.", index_fname.c_str());
            ...
            delete indexfile;
            mlog(SMF_ERR, "Unable to read index entries from %s, stop!",
                 index_fname.c_str());
            ret = -EIO;
            break;
        }
