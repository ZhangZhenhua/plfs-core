...
FileReader::FileReader(const string &fname, int buf_size) {
    filename = fname;
    assert((buf_size & 3) == 0);
...
FileReader::read_buffer() {
    ...
    if (lseek(fd, buffer_pos + buffer_end, SEEK_SET) == (off_t)-1) {
        close(fd);
        return -errno;
    }
    if (buffer_end == buffer_size) {
        ...
        bytes_read = read(fd, &buffer[buffer_end], buffer_size - buffer_end);
        if (bytes_read == -1) {
            close(fd);
            return -errno;
        }
