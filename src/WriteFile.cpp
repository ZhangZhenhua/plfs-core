...
int WriteFile::setOpenFh(OpenFh *ofh, int fh, WriteType wType)
{
...
int WriteFile::addWriter( pid_t pid, bool child , WriteType wType)
{
    ...
    if (ofh != NULL) {
        ...
        if (child==true){
            ...
            ret=Container::makeHostDir(paths, mode, PARENT_ABSENT, NULL,
                                       pid, Util::getTime());
        }
    ...
    switch (wType){
        case SINGLE_HOST_WRITE:
           *index_path = Container::getIndexPath(path,host,p,createtime,
           ...
           break;
        case SIMPLE_FORMULA_WRITE:
           if (index->getFh(SIMPLEFORMULA) != NULL ) return -1;
